...
비즈니스 로직 포함 여부가 불분명한 경우 아래 기준으로 판단한다:

| 의문 상황 | 판단 기준 | 모델 |
| --------- | --------- | ---- |
| "단순 필터링인가, 비즈니스 조건인가?" | disabled/readonly 조건 포함 → 비즈니스 | **sonnet** |
| "날짜 표시인가, 기간 계산인가?" | 덧셈/뺄셈/비교 연산 포함 → 계산 | **opus** |
| "상태 변경인가, 상태 전이인가?" | 특정 조건에서만 전환 가능 → 전이 | **opus** |
| "단순 구현인데 정책이 있는 것 같다" | 기존 코드에 주석/상수로 규칙 표현 → 정책 | **sonnet 이상** |

**요약**: 아래 키워드가 보이면 무조건 상향
