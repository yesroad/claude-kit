...
테스트 파일 존재 여부 확인:
```
├─ 있음 → 기존 테스트 실행만 (생성 단계 스킵)
└─ 없음 또는 --regenerate 옵션 → test-unit 스킬로 생성 후 실행
```
...
```
├─ 있음 → 기존 테스트 실행만 (생성 단계 스킵)
└─ 없음 또는 --regenerate 옵션 → test-integration 스킬로 생성 후 실행
```
...
```
├─ 있음 → 기존 테스트 실행만 (생성 단계 스킵)
└─ 없음 또는 --regenerate 옵션 → test-e2e 스킬로 생성 후 실행
```
