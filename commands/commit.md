...
2. 없으면: `git status --short`로 변경된 파일 확인
   - 변경/untracked 파일 목록을 보여주고 **사용자에게 확인** 요청:
     > "아래 파일들을 커밋할까요? (y/n)"
   - `y` → 해당 파일들 `git add` 후 진행
   - `n` 또는 변경 파일 없음 → `❌ 변경사항이 없습니다.` 출력 후 **중단**
...
- 제목 / Body: **한글**
- **AI 표기 절대 금지**: `Co-Authored-By`, `Co-authored-by`, `🤖`, `Generated with Claude`, `Claude`, `Anthropic` 등 AI 관련 문구를 커밋 메시지 어디에도 포함하지 않는다
