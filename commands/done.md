...
변경된 파일에서 비즈니스 로직 키워드를 탐지한다:

```bash
CHANGED=$(git diff --name-only HEAD)
echo "$CHANGED" | xargs rg "(calculate|compute|format|disabled|isDisabled|readonly|filter|sort|defaultValue|initialValue)" --type ts 2>/dev/null
```
