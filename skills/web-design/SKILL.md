...
**폼 UX 핵심 원칙:**
- **EAS 순서**: Eliminate(불필요 필드 제거) → Automate(자동완성) → Simplify(입력 간소화)
- 단일 컬럼 레이아웃 기본 (멀티 컬럼은 짧은 관련 필드 예외)
- 오류 표시 타이밍: blur 이후만 (타이핑 중 즉시 표시 = 적대적 패턴)
...
**UX 체크 (랜딩페이지·폼·네비게이션 포함 시):**

- 히어로 섹션에서 사이트 목적을 50ms 안에 파악할 수 있는가?
- 데스크톱에서 햄버거 메뉴를 사용하지 않았는가?
- CTA 버튼 카피가 클릭 후 일어날 일을 구체적으로 설명하는가?
- 동일한 시각적 무게의 CTA가 2개 이상 없는가?
- 폼이 있다면 blur 이후에만 오류가 표시되는가?
- 오류 표시가 색상 단독이 아닌 색상 + 아이콘 + 테두리 조합인가?
...
- **UX 패턴 (CTA, 폼, 네비게이션, 인지 부하, 마이크로카피)**: `references/ux-patterns.md`
